...
export function makeCloakMesh(
	innerRadius: number,
	thickness: number,
	uTesselation: number,
): TriangleSoup {
	const vTesselation = uTesselation * 4;

	const posY = new Coord3({ x: 0, y: 1, z: 0 });
	const negX = new Coord3({ x: -1, y: 0, z: 0 });
	const posZ = new Coord3({ x: 0, y: 0, z: 1 });
	const startingPoint = negX.scaled(innerRadius + thickness);
	...
	// Half sphere - cap.
	{
		for (let v = 0; v <= vTesselation; ++v) {
			const vFactor = v / vTesselation;

			sphereVertices[v] = [];
			for (let u = 0; u <= uTesselation; ++u) {
				const uFactor = u / uTesselation;
	...
	// Rim
	{
		const rimRadius = thickness / 2;
		const rimCenter = posZ.scaled(innerRadius + rimRadius);

		const rimVertices: Array<Array<Vertex>> = [];
		const rimTesselation = uTesselation;
		for (let v = 0; v <= vTesselation; ++v) {
			const vFactor = v / vTesselation;
...
function axisRotate(axis: Coord3, coord: Coord3, angleInDeg: number) {
